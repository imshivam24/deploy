...
def inp_file_gen_multiple(uploaded_file, children_folder):
    ...
    if uploaded_file:
        try:
            # Read necessary sheets from the Excel file
        ...
        try:
            ...
            concentrations = read_formulas(uploaded_file, 'Input-Output Species', 'Input MKMCXX')['Input MKMCXX']
            rxn = data1["Reactions"]
            Ea = read_formulas(uploaded_file, 'Reactions', 'G_f')['G_f']
            Eb = read_formulas(uploaded_file, 'Reactions', 'G_b')['G_b']
            P = data2["Pressure"][0]
